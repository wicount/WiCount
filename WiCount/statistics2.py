...
def getBuildings():
    ...
    rows = cur.execute('''SELECT DISTINCT(building) FROM room''').fetchall()
...
def getPerUtal():
    ...
    rows = cur.execute('''SELECT  room_id, PredictedPercentage, count(*) as count, room From analytics GROUP BY room_id, PredictedPercentage''').fetchall()
